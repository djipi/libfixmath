...
BLITTER_BSS_RAZ		=	0				; 0: No blitter usage for BSS initialisation (TBD)
IRQ					=	1				; 0: No interrupt
RELOCATION_DATA		=	0				; 0: No data relocation
TEST_M68KEXCEPTIONS	=	0				; 1: Test M68K exceptions
TEST_ROM			=	0				; 1: Test ROM writing
TEST_OUTOFROMROM	=	0				; 1: Test out of ROM access
;
.if	ATARI_SETUP	= 1
...
;	.extern	__bss_end__
	.extern	__bss_size__
	.extern	__bss_start_PHR__
	.extern __bss_size_CPHR__
.endif
	.extern __Stack
.if	UNDERSCORE_DEF = 1
	.extern	_main
...
.if TEST_ROM = 1
	move.b	#$01, $800000
	move.w	#$0123, $800000
	move.l	#$01234567, $800000
.endif
;
; Test out of ROM writing
;
.if TEST_OUTOFROMROM = 1
	move.b	#$01, $FF800000
	move.w	#$0123, $FF800000
	move.l	#$01234567, $FF800000
.endif
;
; Relocate data
;
...
.else
.bwait:
   move.l   B_CMD, d0									; check blitter availability
   ror.w    #1, d0
   bcc.b    .bwait
   move.l   #__bss_start_PHR__, A1_BASE    				; BSS pointer
   move.l   #0, A1_PIXEL         						; pointer destination offset (must be null as we start at the BSS address)
   ;move.l   #(PIXEL16|XADDPHR|PITCH1), A1_FLAGS		; use 2 bytes, phrase mode and no phrase gap
   move.l   #(PIXEL32|XADDPHR|PITCH1), A1_FLAGS			; use 4 bytes, phrase mode and no phrase gap
   ;move.l   #(PIXEL8|XADDPIX|PITCH1), A1_FLAGS			; use 1 byte, pixel mode and no phrase gap
   move.l   #0, B_PATD									; value to fill
   ;move.l   #0, B_PATD+4
   ;move.l	#$4ffff, A1_STEP
   move.l   #__bss_size_CPHR__, B_COUNT					; set the counts value
   ;move.l   #$00043000, B_COUNT
   move.l   #PATDSEL, B_CMD       						; use B_PATD and launch the memset
